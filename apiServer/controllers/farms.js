...
module.exports = class farms {
  ...
  static activity(){
    ...
    let query = "INSERT INTO farms (information,summary,activity,stage,weeks,image_updates,date,farm_id) VALUES (?,?,?,?,?,?,?,?)";
    let values = [
  ...
  static updateActivity(req,res){
    ...
    values.push(req.query.activityId);
  ...
  static allActivity(req,res){
    ...
    let query   = `SELECT farm_activities.*,status.name as status_name,states.name as location_name,farms.* FROM farms INNER JOIN status ON status.id = farms.status INNER JOIN farms on farms.id = farm_activities.farm_id ${operate(req).on('title').on('category').on('total_units').on('price_per_units').on('funding_starts').on('funding_ends').on('farm_starts').on('farm_ends').on('roi').on('location').on('status').done()} ${req.paginate(20)}`;
